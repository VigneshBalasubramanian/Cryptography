...
public final class Rijndael {
     ...
     */
    private void makeKey(byte[] cipherKey, int keyBits, int direction)
     ...
     * @param   keyBits     size of the cipher key in bits.
     */
    private void makeKey(byte[] cipherKey, int keyBits)
            throws RuntimeException {
     ...
     */
    private void encrypt(byte[] pt, byte[] ct) {
        /*
     ...
     * @param   pt          plaintext block.
     */
    private void decrypt(byte[] ct, byte[] pt) {
        /*
	     * map byte array block to cipher state
	     * and add initial round key:
	     */
    ...
    public byte[] encrypt(byte[] msg, byte[] key, int keySize) {
    	int mod = msg.length % Rijndael.BLOCK_SIZE;
    	int block = msg.length / Rijndael.BLOCK_SIZE;
    	int length = msg.length;
    	if (mod > 0) {
    		length = length + (Rijndael.BLOCK_SIZE - mod);
    	}
        byte[] ct = new byte[length];

        makeKey(key, keySize);
        for (int i = 0; i < block; i++) {
            byte[] in = new byte[Rijndael.BLOCK_SIZE];
            byte[] out = new byte[Rijndael.BLOCK_SIZE];
            System.arraycopy(msg, i*Rijndael.BLOCK_SIZE, in, 0, Rijndael.BLOCK_SIZE);
            encrypt(in, out);
            System.arraycopy(out, 0, ct, i*Rijndael.BLOCK_SIZE, Rijndael.BLOCK_SIZE);
        }
    	if (mod > 0) {
            byte[] in = new byte[Rijndael.BLOCK_SIZE];
            byte[] out = new byte[Rijndael.BLOCK_SIZE];
            System.arraycopy(msg, block*Rijndael.BLOCK_SIZE, in, 0, mod);
            encrypt(in, out);
            System.arraycopy(out, 0, ct, block*Rijndael.BLOCK_SIZE, Rijndael.BLOCK_SIZE);
    	}
    	return ct;
    }
    ...
    public byte[] decrypt(byte[] msg, byte[] key, int keySize) {
        ...
        System.arraycopy(ct, 0, result, 0, i + 1);
    	return result;
    }
}
